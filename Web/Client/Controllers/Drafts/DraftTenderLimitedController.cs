...
{
    [Authorize]
    [RoutePrefix("draft/tenderLimited")]
    public class DraftTenderLimitedController : BaseDraftController
    {
        private IDraftProvider DraftProvider { get; }

        public DraftTenderLimitedController(IDraftProvider draftProvider, Lazy<IAccessManager> accessManager)
            : base(accessManager)
        {
        ...
        {
            var viewModel = new DraftTenderLimitedViewModel();
            return View(viewModel);
        }

        [Route("createTender")]
        [HttpPost]
        public async Task<ActionResult> AddTender(DraftTenderLimitedViewModel viewModel)
        {
        ...
        [Route("editTender/{tenderGuid:guid}")]
        [HttpPost]
        public async Task<ActionResult> EditTender(DraftTenderLimitedViewModel viewModel)
        {
            if (!ModelState.IsValid)
            {
                return View(viewModel);
