...
{
    [Authorize]
    [RoutePrefix("draft/tenderDefense")]
    public class DraftTenderDefenseController : BaseDraftController
    {
        private IDraftProvider DraftProvider { get; }

        public DraftTenderDefenseController(IDraftProvider draftProvider, Lazy<IAccessManager> accessManager)
            : base(accessManager)
        {
        ...
        {
            var viewModel = new DraftTenderDefenseViewModel();
            return View(viewModel);
        }

        [Route("createTender")]
        [HttpPost]
        public async Task<ActionResult> AddTender(DraftTenderDefenseViewModel viewModel)
        {
        ...
        [Route("editTender/{tenderGuid:guid}")]
        [HttpPost]
        public async Task<ActionResult> EditTender(DraftTenderDefenseViewModel viewModel)
        {
            if (!ModelState.IsValid)
            {
                return View(viewModel);
