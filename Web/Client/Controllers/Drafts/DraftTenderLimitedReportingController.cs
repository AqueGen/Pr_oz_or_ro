...
{
    [Authorize]
    [RoutePrefix("draft/tenderLimitedReporting")]
    public class DraftTenderLimitedReportingController : BaseDraftController
    {
        private IDraftProvider DraftProvider { get; }

        public DraftTenderLimitedReportingController(IDraftProvider draftProvider, Lazy<IAccessManager> accessManager)
            : base(accessManager)
        {
        ...
        {
            var viewModel = new DraftTenderLimitedReportingViewModel();
            return View(viewModel);
        }

        [Route("createTender")]
        [HttpPost]
        public async Task<ActionResult> AddTender(DraftTenderLimitedReportingViewModel viewModel)
        {
        ...
        [Route("editTender/{tenderGuid:guid}")]
        [HttpPost]
        public async Task<ActionResult> EditTender(DraftTenderLimitedReportingViewModel viewModel)
        {
            if (!ModelState.IsValid)
            {
                return View(viewModel);
