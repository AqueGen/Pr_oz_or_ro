...
{
    [Authorize]
    [RoutePrefix("draft/tenderLimitedQuick")]
    public class DraftTenderLimitedQuickController : BaseDraftController
    {
        private IDraftProvider DraftProvider { get; }

        public DraftTenderLimitedQuickController(IDraftProvider draftProvider, Lazy<IAccessManager> accessManager)
            : base(accessManager)
        {
        ...
        {
            var viewModel = new DraftTenderLimitedQuickViewModel();
            return View(viewModel);
        }

        [Route("createTender")]
        [HttpPost]
        public async Task<ActionResult> AddTender(DraftTenderLimitedQuickViewModel viewModel)
        {
        ...
        [Route("editTender/{tenderGuid:guid}")]
        [HttpPost]
        public async Task<ActionResult> EditTender(DraftTenderLimitedQuickViewModel viewModel)
        {
            if (!ModelState.IsValid)
            {
                return View(viewModel);
