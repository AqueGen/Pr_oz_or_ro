...
namespace Kapitalist.Web.Framework.Helpers
{
    public static class CultureHelper
    {
        ...
        public const string TIMEZONEINFO = "TimeZoneInfo";

        public static TimeZoneInfo GetLocalTimeZoneInfo()
        {
            return null;
            HttpContext contect = HttpContext.Current;
            if (contect == null)
                return null;
            TimeZoneInfo timeZoneInfo = contect.Session[TIMEZONEINFO] as TimeZoneInfo;
            if (timeZoneInfo == null)
            {
                var keyCookie = contect.Request.Cookies[TIMEZONENAME];
                if (keyCookie != null
                    && TimeZoneMapper.TimeZoneMap.DefaultValuesTZMapper.TryMapTZID(keyCookie.Value, out timeZoneInfo))
                    contect.Session[TIMEZONEINFO] = timeZoneInfo;
            }
            return timeZoneInfo;
        }

        public static MvcHtmlString FormatDateTime(this HtmlHelper htmlHelper, DateTime? dateTime, string format = "g", bool jsCorrection = true)
        {
            string value = $"<span>{GlobalRes.Absent}</span>";
            if (dateTime.HasValue)
            {
                value = dateTime.Value.ToLocalString(format);
                if (jsCorrection)
                {
                    value = $"<span data-date='{JsDate(dateTime)}' data-date-format='{JsDateFormat(format)}'>{value}</span>";
                }
            }
        ...
        /// <summary>
        /// Якщо серверу відома часова зона клієнта - повертає локальний час,
        /// якщо ні - в кінці значення ставиться помітка UTC
        /// </summary>
        /// <param name="dateTime"></param>
        /// <param name="format"></param>
        /// <returns></returns>
        public static string ToLocalString(this DateTime dateTime, string format = "g")
        {
            string zone = " UTC";
            TimeZoneInfo timeZoneInfo = GetLocalTimeZoneInfo();
            if (timeZoneInfo != null)
            {
                dateTime = TimeZoneInfo.ConvertTimeFromUtc(dateTime, timeZoneInfo);
                zone = "";
            }
            return dateTime.ToString(format) + zone;
        }

        public static double? JsDate(this DateTime? dateTime)
        {
            return dateTime.HasValue ? dateTime.Value.Subtract(new DateTime(1970, 1, 1)).TotalMilliseconds : (double?)null;
        }

        public static string JsDateFormat(string serverFormat)
        {
            switch (serverFormat)
            {
                case "D":
                ...
                default:
                    return serverFormat.Replace('d', 'D').Replace('y', 'Y');
            }
        }
    }
}